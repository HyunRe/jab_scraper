...
class IncruitCollector(JobCollectorRepository):
    ...
    def _format_deadline(self, deadline_str: str) -> str:
        ...
        # 3. 등록 후 경과 시간 표현(예: '3일 전') 및 상시 채용 키워드 처리
        if any(keyword in cleaned for keyword in ["전", "일 전", "시간 전", "분 전"]):
            return "상시 채용"

        if any(
                keyword in cleaned for keyword in ["상시", "채용시", "채용 시", "9999"]
        ):
            return "상시 채용"
        ...
        try:
            ...
            # 월/일 및 한글 날짜 형식 (예: 04/15, 04-15, 4월 15일)
